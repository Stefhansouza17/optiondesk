...
function Home({ assets, onSelectAsset, onShowPositions }) {
  ...
  const priceRows=chain.filter(o=>o.option_type===optType&&etfPrice>0&&o.strike>=etfPrice*0.85&&o.strike<=etfPrice*1.15).map(o=>o.strike).sort((a,b)=>b-a);
  return (
...
function ClaudeChat({ assets, onSaveTrade }) {
  ...
  const confirmTrades = async () => {
    ...
    if(notFound.length>0){
      setMessages(p=>[...p,{role:"assistant",content:`✅ ${saved} trade(s) saved! ⚠️ Asset not found: ${notFound.join(", ")} — make sure it exists in your dashboard.`}]);
    } else {
      setMessages(p=>[...p,{role:"assistant",content:`✅ ${saved} trade${saved>1?"s":""} saved successfully!`}]);
    }
  };
