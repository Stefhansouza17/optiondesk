...
const INITIAL_ASSETS = [
  { id:"IBIT", ticker:"IBIT", strategy:"PMCC", color:"#00d4aa", leapStrike:37, leapExpiration:"Jan 2027", leapCost:8.385, leapContracts:2, leapDelta:0.71, initialPrice:41.55, active:true,
    trades:[
  ...
  { id:"EWZ", ticker:"EWZ", strategy:"PMCC", color:"#f5c842", leapStrike:29, leapExpiration:"Nov 2026", leapCost:9.34, leapContracts:1, leapDelta:0.65, initialPrice:33.5, active:true,
    trades:[
...
function AssetDashboard({ asset, onUpdate, onClose }) {
  ...
  const totalLeapCost = asset.leapCost*(asset.leapContracts||1);
  const recovPct = Math.min(totalCollected/totalLeapCost,1);
  ...
  function confirmClose(){
    ...
    const today=new Date().toISOString().slice(0,10);
    let upd=trades.map(t=>t.status==="open"?{...t,status:"closed"}:t);
    upd=[...upd,{id:Date.now(),date:today,action:"BUY",strike:0,expiration:today,premium:parseFloat(closeForm.closePrem),contracts:1,status:"closed",note:"LEAP closed"}];
    setTrades(upd);
  ...
  return (
    <div>
      ...
      <div className="pbar">
        ...
        {isPremium&&<><div className="dvdr"/><div className="sml">LEAP <span>${asset.leapStrike}</span></div><div className="dvdr"/><div className="sml">distance <span className={etfPrice>=asset.leapStrike?"green":"red"}>{etfPrice>=asset.leapStrike?"+":""}{fmt(etfPrice-asset.leapStrike)}</span></div></>}
          ...
          <>
            <div className="cards">
              {(isPremium?[
                ...
                ["#3a8fff","Current cost basis",`$${fmt(costBasis)}`,`was $${asset.leapCost}`],
                ["#f5c842","Open positions",openTrades.filter(t=>t.action==="SELL").length,`${closedTrades.length} closed`],
                ["#ff4d6a","Accumulated P&L",`${totalDollar>=0?"+":""}$${fmt(totalDollar)}`,`${fmt(totalCollected/asset.leapCost*100)}% return`],
              ]:[
              ...
              <>
                <div className="sec">
                  <div className="sechdr">
                    ...
                    <div style={{fontSize:11,color:"#5a7a9a"}}>target: <span style={{color:"#c8d8e8"}}>${fmt(asset.leapCost*(asset.leapContracts||1)*100)}</span></div>
                  </div>
                  <div style={{padding:"14px 16px"}}>
                    <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:"#5a7a9a",marginBottom:8}}>
                      ...
                      <span>${fmt(asset.leapCost*(asset.leapContracts||1)*100)}</span>
                    </div>
                    <div className="ptrack"><div className="pfill" style={{width:`${recovPct*100}%`,background:`linear-gradient(90deg,${color},#3a8fff)`}}/></div>
                    <div style={{fontSize:11,color:"#5a7a9a",marginTop:6}}>$<span style={{color:"#f5c842"}}>{fmt(Math.max(asset.leapCost*(asset.leapContracts||1)*100-totalDollar,0))}</span> remaining to free LEAP</div>
                  </div>
                </div>
                <div className="sec">
                  <div className="sechdr">
                    <div className="sectitle">LEAP position</div>
                    <div style={{display:"flex",gap:8,alignItems:"center"}}>
                      {(asset.leapContracts||1)>1&&<span style={{fontSize:11,color:"#f5c842"}}>{asset.leapContracts} contracts · avg ${fmt(asset.leapCost)}</span>}
                  ...
                  <div className="lgrid">
                    {[
                      ["Asset", asset.ticker],
                      ["Strike", `$${asset.leapStrike}`],
                      ["Expiration", asset.leapExpiration],
                      ["Avg cost", `$${fmt(asset.leapCost)}`],
                      ["Contracts", asset.leapContracts||1],
                      ["Total cost", `$${fmt(asset.leapCost*(asset.leapContracts||1)*100)}`],
                      ["Current basis", `$${fmt(costBasis)}`],
                      ["Delta", asset.leapDelta],
                      ["Recovered", `${fmt(recovPct*100,1)}%`],
                    ].map(([l,v])=>(
...
function Home({ assets, onSelectAsset, onShowPositions }) {
  ...
  const totals = useMemo(()=>assets.filter(a=>a.active).map(a=>{
    ...
    return {...a,col,colDollar:col*100,basis:a.leapCost-col,openTrades,openSells,openPremium,nearestExp,daysLeft};
  }),[assets]);
  const grandCol=useMemo(()=>totals.reduce((a,t)=>a+t.colDollar,0),[totals]);
  const grandCost=useMemo(()=>totals.reduce((a,t)=>a+t.leapCost*(t.leapContracts||1)*100,0),[totals]);
  const openPositions=useMemo(()=>totals.reduce((a,t)=>a+t.openTrades.length,0)+totals.filter(t=>t.leapCost>0).reduce((a,t)=>a+(t.leapContracts||1),0),[totals]);
  ...
  const filteredTotals=useMemo(()=>totals
    ...
    .sort((a,b)=>{
      ...
      if(sortBy==="recovery")return(b.col/b.leapCost)-(a.col/a.leapCost);
      return 0;
  ...
  return (
    <div className="main fade-in">
      ...
      {totals.length>0&&(
        <div className="sec" style={{marginBottom:24}}>
          <div className="sechdr">
            <div className="sectitle">Premium timeline <Tooltip text="Visual history of premium collected per trade across all assets."/></div>
          </div>
          <div style={{padding:"16px",overflowX:"auto"}}>
            {totals.map(t=>{
              const sells = t.trades.filter(tr=>tr.action==="SELL").sort((a,b)=>new Date(a.date)-new Date(b.date));
              if(!sells.length) return null;
              const maxPrem = Math.max(...sells.map(s=>parseFloat(s.premium||0)*parseInt(s.contracts||1)*100));
              return (
                <div key={t.id} style={{marginBottom:16}}>
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                    <span style={{fontFamily:"Syne,sans-serif",fontWeight:700,fontSize:13,color:"#fff"}}>{t.ticker}</span>
                    <StratBadge strategy={t.strategy||"PMCC"}/>
                    <span style={{fontSize:11,color:"#5a7a9a",marginLeft:"auto"}}>{sells.length} trades · ${fmt(t.colDollar)} total</span>
                  </div>
                  <div style={{display:"flex",alignItems:"flex-end",gap:4,height:64}}>
                    {sells.map((s,i)=>{
                      const val=parseFloat(s.premium||0)*parseInt(s.contracts||1)*100;
                      const h = Math.max(val/maxPrem*100,8);
                      const retPct=(parseFloat(s.premium||0)/t.leapCost*100).toFixed(1);
                      const isOpen = s.status==="open";
                      return (
                        <div key={s.id} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:2,flex:"0 0 auto"}}>
                          <span style={{fontSize:8,color:isOpen?t.color:"#3a5a7a",fontWeight:isOpen?700:400,letterSpacing:0.2,lineHeight:1}}>{retPct}%</span>
                          <div title={`$${fmt(val)} · ${retPct}% · ${s.date}`} style={{
                            width:14,height:`${h}%`,minHeight:4,
                            background:isOpen?`linear-gradient(180deg,${t.color},${t.color}88)`:`${t.color}55`,
                            borderRadius:"2px 2px 0 0",
                            border:isOpen?`1px solid ${t.color}`:"none",
                            cursor:"pointer",
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",fontSize:10,color:"#3a5a7a",marginTop:4}}>
                    <span>{sells[0]?.date}</span>
                    <span style={{color:t.color}}>brighter = open</span>
                    <span>{sells[sells.length-1]?.date}</span>
                  </div>
                </div>
              );
            })}
      ...
      {sym&&(
        <>
          ...
          <div style={{display:"flex",gap:12,marginBottom:16,alignItems:"center",flexWrap:"wrap"}}>
            ...
            <div style={{fontFamily:"Syne",fontSize:16,fontWeight:700,color:optType==="call"?"#3a8fff":"#a78bfa"}}>${strike.toFixed(1)}{optType==="call"?"C":"P"}</div>
      ...
      {!sym&&(<div style={{textAlign:"center",padding:"40px 0",color:"#3a5a7a",fontSize:12,letterSpacing:0.5}}>Type a ticker above to simulate a strategy</div>)}
    </div>
  );
}
...
function AllPositionsModal({ assets, onClose }) {
  const allOpen = assets.filter(a=>a.active).flatMap(a=>{
    const rows = [];
    // LEAPs — one row per contract
    const contracts = a.leapContracts||1;
    for(let i=0;i<contracts;i++){
      rows.push({
        type:"LEAP", ticker:a.ticker, color:a.color,
        strike:`$${a.leapStrike}`, expiration:a.leapExpiration,
        cost:`$${fmt(a.leapCost)}`, contracts:1,
        action:"BUY", status:"open", premium:"—",
      });
    }
    // Open short calls
...
function App() {
  ...
  const closeAsset = (id, finalTrades) => {
  ...
  const handleImport = () => importJSON((a,c)=>{ setAssets(a); setClosedAssets(c); setActive("home"); });
    ...
    <div style={{minHeight:"100vh",background:"#080c10",color:"#c8d8e8"}}>
      ...
      <div className="hdr">
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          <button className="btn bsm bneutral" onClick={handleImport} title="Restore from backup">↑ Restore</button>
          <button className="btn bsm bneutral" onClick={()=>exportJSON(assets,closedAssets)} title="Backup all data">↓ Backup</button>
        </div>
      </div>
